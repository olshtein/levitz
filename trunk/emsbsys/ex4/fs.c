...
FS_STATUS fs_init(const FS_SETTINGS settings){
	...
	_lastFile=0;
	_flash[_currentHalf]._headerFiles_num=0;
	_flash[_currentHalf]._headerStartPos=sizeof(Signature);
	_flash[_currentHalf]._next_avilable_header_pos=sizeof(Signature);
	_flash[_currentHalf]._dataStartPos=(uint16_t)(HALF_SIZE-1);
	_flash[_currentHalf]._next_avilable_data_pos=(uint16_t)(HALF_SIZE-1);
	void *t =(firstHalf+1);
